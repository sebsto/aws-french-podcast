...
async function loadAnalytics() {
  try {
    // Determine base URL from the page's script tags
    const scripts = document.querySelectorAll('script[src*="stats.js"]');
    let baseUrl = '';
    if (scripts.length > 0) {
      const src = scripts[0].getAttribute('src');
      baseUrl = src.replace('/js/stats.js', '');
    }
    
    const response = await fetch(`${baseUrl}/data/analytics.json`);
